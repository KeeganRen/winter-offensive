...
namespace svo {
    ...
    size_t SparseImgAlign::run(FramePtr ref_frame, FramePtr cur_frame)
    {
        ...
        else
        {
            ...
            {
                SVO_WARN_STREAM("SparseImgAlign: no depth map to track!");
                return 0;
            }
    ...
    void SparseImgAlign::precomputeReferencePatches()
    {
        ...
        if (cur_frame_->sparse_aligned_) 
        {
                    ...
                    it!=ite; ++it, ++feature_counter, ++visiblity_it)
            {
                ...
                if(sqrt(it->second->sigma2) < 0.005*it->second->z_range || u_ref_i-border < 0 || v_ref_i-border < 0 || u_ref_i+border >= ref_img.cols || v_ref_i+border >= ref_img.rows)
                    continue;
                *visiblity_it = true;
                visi_num ++;
